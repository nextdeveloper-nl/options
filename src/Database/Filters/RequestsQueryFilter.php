...
class RequestsQueryFilter extends AbstractQueryFilter
{
    ...
    {
        return $this->builder->where('uri', 'ilike', '%' . $value . '%');
    }
    ...
    {
        return $this->builder->where('controller', 'ilike', '%' . $value . '%');
    }

    public function topic($value)
    {
        return $this->builder->where('topic', 'ilike', '%' . $value . '%');
    }

    public function controllerDescription($value)
    {
        return $this->builder->where('controller_description', 'ilike', '%' . $value . '%');
    }

    public function actionDescription($value)
    {
        return $this->builder->where('action_description', 'ilike', '%' . $value . '%');
    }
